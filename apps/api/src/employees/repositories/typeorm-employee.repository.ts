...
export class TypeOrmEmployeeRepository implements IEmployeeRepository {
  ...
  async create(createDto: CreateEmployeeDto): Promise<Employee> {
    // Convert the DTO to entity data
    const entityData = this.mapper.createDtoToEntity(createDto);

    // Create a new employee entity (ID will be generated by the database)
  ...
  async addMany(employees: Employee[]): Promise<void> {
    const entities = employees.map(emp => 
      this.employeeRepository.create(this.mapper.domainToEntity(emp))
    );
    await this.employeeRepository.save(entities);
  }
  ...
  async countWithFilter(filters?: Partial<Record<keyof Employee, any>>): Promise<number> {
    if (!filters || Object.keys(filters).length === 0) {
      return this.count();
    }

    // Convert filters to TypeORM format
    ...
    return await this.employeeRepository.count({ where: whereCondition });
  }
  ...
  async countByStatus(status?: string): Promise<number> {
    ...
    // Use a query builder to avoid TypeORM type issues
  ...
  async countUniqueDepartments(): Promise<number> {
    // Use a query that counts distinct departmentIds
